...
import { CheckCircle2, Lock, Sparkles, Check, RefreshCw, Film, Calendar } from 'lucide-react';
...
export default function DynamicMockup({ activeStageId, activeBrand, selectedPrompt }: DynamicMockupProps) {
  ...
  // Typewriter effect simulation for Slide 6 verification recovery.
  useEffect(() => {
    if (activeStageId === 5) {
      const promptToType = selectedPrompt || 'Check the live public URL.';
      setTypedPrompt('');
      let i = 0;
  ...
  // Slide 6: Publication verification and recovery mockup.
  const renderVerificationRecovery = () => {
    return (
        ...
        <div className="md:col-span-5 space-y-3 bg-black/60 border border-white/5 p-3 rounded-xl font-mono text-[9px] flex flex-col justify-between">
          <div className="space-y-2">
            <div className="flex justify-between items-center border-b border-white/5 pb-1">
              <span className="text-zinc-500">PUBLICATION CHECK V0</span>
              <span className="text-zinc-500">RECONCILE_ENGINE</span>
            </div>
            <div className="space-y-2 leading-tight">
              <p className="text-zinc-400">sakhaa-forge:~ $ verify-post --instruction</p>
              ...
              {typedPrompt.length > 0 && (
                ...
                >
                  <p>&gt; Verification instruction received</p>
                  <p>&gt; Publication evidence checked</p>
                  <p>&gt; Recovery state updated</p>
                </motion.div>
              )}
            </div>
          </div>
          <div className="text-[8px] text-zinc-500 border-t border-white/5 pt-1.5 leading-none">
            Verifier: v0.1.0 (contract bound)
          </div>
        </div>

        {/* Publication evidence comparison */}
        <div className="md:col-span-7 space-y-3 bg-black/40 border border-white/5 p-3 rounded-xl flex flex-col justify-between">
          <span className="px-1.5 py-0.5 text-[8px] font-mono bg-zinc-900 border border-white/10 text-zinc-400 rounded self-start">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-[9px] font-mono">
              <div className="bg-white/2 border border-white/5 p-2 rounded-lg">
                <span className="text-zinc-500 uppercase">Provider acknowledgement</span>
                <p className="text-xs font-bold text-zinc-300 mt-1">Accepted, pending check</p>
                <div className="h-1 w-full bg-white/5 rounded mt-1.5 overflow-hidden">
                  <div className="h-full bg-zinc-500" style={{ width: '72%' }} />
                </div>
              </div>
              <div className="bg-white/2 border border-white/5 p-2 rounded-lg" style={{ borderColor: `${activeBrand.primaryColor}33` }}>
                <span style={{ color: activeBrand.primaryColor }}>Audience verification</span>
                <p className="text-xs font-bold text-emerald-400 mt-1">Published and verified</p>
                <div className="h-1 w-full bg-white/5 rounded mt-1.5 overflow-hidden">
            ...
            <div className="space-y-1.5 font-mono text-[8px] text-zinc-400">
              <div className="space-y-1 border-t border-white/5 pt-2">
                <div className="flex justify-between"><span>Live URL:</span> <span className="text-emerald-400">Resolved</span></div>
                <div className="flex justify-between"><span>Media fingerprint:</span> <span className="text-emerald-400">Matched</span></div>
                <div className="flex justify-between"><span>Ledger state:</span> <span className="text-white">Reconciled</span></div>
              </div>
            </div>
          </div>
  ...
  const renderStageVisual = () => {
    switch (activeStageId) {
      ...
      case 5:
        return renderVerificationRecovery();
      default:
        return (
