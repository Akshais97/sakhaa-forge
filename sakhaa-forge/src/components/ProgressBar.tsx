...
export default function ProgressBar({ stages, activeStageId, activeBrand, onSelectStage }: ProgressBarProps) {
  return (
    <div className="shrink-0 w-full border-t border-white/5 bg-zinc-950/80 backdrop-blur-xl px-5 py-2.5 flex flex-col space-y-2 select-none" id="production-timeline-rail">
      ...
      <div className="flex justify-between items-center text-[9px] font-mono text-zinc-500 uppercase tracking-widest px-1">
        ...
        <span>Jump to any step</span>
      </div>

      {/* Horizontal Scrollable timeline track */}
      <div className="relative flex items-center justify-between overflow-x-auto gap-3 py-1.5 no-scrollbar scroll-smooth" id="timeline-rail-scroll">
        ...
        {stages.map((stage) => {
          ...
          return (
            ...
            >
              ...
              <div
                className="h-6 w-6 rounded-full border flex items-center justify-center font-mono text-[9px] font-bold transition-all duration-300 bg-zinc-950"
                style={{
              ...
              >
                {stage.id + 1 < 10 ? `0${stage.id + 1}` : stage.id + 1}
              </div>

              {/* Label below */}
              <span
                className={`hidden sm:block text-[8px] font-mono tracking-wider uppercase mt-1.5 transition-colors duration-200 ${
                  isActive
