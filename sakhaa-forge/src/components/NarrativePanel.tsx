...
export default function NarrativePanel({
  ...
  const isV2Simulation = activeStage.key === 'v2-simulation';

  return (
    <div className={`flex flex-col justify-between h-full text-left ${isV2Simulation ? 'space-y-2' : 'space-y-4'}`} id="narrative-panel">
      {/* Header tagline metadata */}
      <div className={isV2Simulation ? 'space-y-2' : 'space-y-3'}>
        ...
        <h3 className={`${isV2Simulation ? 'text-xl md:text-2xl' : 'text-2xl md:text-[1.8rem]'} font-display font-medium text-white tracking-tight leading-tight text-balance`}>
          {activeStage.title}
        </h3>

        {/* Main description block */}
        <p className={`${isV2Simulation ? 'text-xs leading-snug' : 'text-sm leading-relaxed'} text-zinc-300/90 font-sans text-pretty`}>
      ...
      {activeStage.body && (
        <p className={`${isV2Simulation ? 'text-[11px] leading-snug' : 'text-xs leading-relaxed'} text-zinc-400 font-sans border-l pl-3 whitespace-pre-line text-pretty`} style={{ borderColor: `${activeBrand.primaryColor}55` }}>
          {activeStage.body}
        </p>
      )}
      ...
      {activeStage.uiLabels && activeStage.uiLabels.length > 0 && (
        <motion.div
          ...
          className={`${isV2Simulation ? 'p-2.5 space-y-2' : 'p-3 space-y-2.5'} rounded-xl border border-white/5 bg-zinc-950/40`}
        >
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 gap-x-4 ${isV2Simulation ? 'gap-y-1 text-[11px]' : 'gap-y-1.5 text-xs'} text-zinc-400`}>
      ...
      {/* Traversal buttons */}
      <div className={`flex items-center gap-3 ${isV2Simulation ? 'pt-1' : 'pt-2'}`}>
        {!isFirstWorkflowStage && (
          <button
            onClick={onPrevStage}
